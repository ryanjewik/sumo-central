...
if webhook['type'] == 'matchResults':
    try: #postgres connection
        ...
        for match in webhook['payload_decoded']:
            #gets rikishi ids for running matches api calls
            if match['eastId'] not in rikishi_list:
                rikishi_list.append(match['eastId'])
            if match['westId'] not in rikishi_list:
                rikishi_list.append(match['westId'])
            ...
            if match['winnerId'] == match['eastId']:
                ...
                WHERE id = %s
                """
                cur.execute(query, (match['eastId'],))
                ...
                WHERE id = %s
                """
                cur.execute(query, (match['westId'],))
            else:
                query = """
                UPDATE rikishi
                SET matches = matches + 1,
        ...
        for rikishi in rikishi_list:
            print(f"Rikishi profile updated for ID: {rikishi}")
